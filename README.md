...
Each project and experience entry on the Projects and Experiences page is driven by structured data files, making it easy to add new content without touching the component logic. There's also a secret page only reachable by solving the capture the flag embedded into the About page. The site is deployed on Vercel with automatic redeployment on every push to the main branch.
